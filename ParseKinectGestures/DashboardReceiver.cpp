...
Robot1073::ProcessDashboardData(char *rxRequest)
{
	...
	int y1to6;
	int index;
	char rxRequestCopy[256];
	rxRequestCopy[0] = '\0';
	strcpy(rxRequestCopy, rxRequest);
	...
	case 'K':
		{		
			int clawstate;
			clawstate = atoi(strtok ( NULL , "\t"));
			if(clawstate == 0){}
				//pincer->Open();			//call open
			else if (clawstate == 1){}
				//pincer->Close();		//call close
			int old = clawstate;
			clawstate = (clawstate > 1 || clawstate < 0 ) ? 1: old;
	...
	case 'L':
		{
			int pegNumber;
			pegNumber = atoi(strtok ( NULL , "\t")); // needs a 1-6 because Float 0.0 is peg 1
			if(pegNumber == 0)
				elevator->GoToPositionFeet(0.0f);
			else
